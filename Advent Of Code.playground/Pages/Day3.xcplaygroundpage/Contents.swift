...
func main(_ data: String) {
    ...
    let firstResult = calculateFirst(matrix)
    let secondResult = findGears(matrix)
    
    print("""
          The sum of all of the part numbers in the engine schematic: - \(firstResult)
          The sum of all of the gear ratios in your engine schematic - \(secondResult)
          """)
}

// MARK: - Task 1

func calculateFirst(_ matrix: [[Character]]) -> Int {
    ...
    while row < matrix.count {
        ...
        while col < matrix[row].count {
            ...
            if ch.isDigit {
                let startIndex = col
                let endIndex = getEndIndex(from: startIndex, in: matrix[row])
                
                if check(matrix, startingFrom: startIndex, upTo: endIndex, atRow: row) {
                    if let num = Int(String(matrix[row][startIndex...endIndex])) {
                        sum += num
                    }
                }
                col = endIndex + 1
...
func getEndIndex(from index: Int, in line: [Character]) -> Int {
    var result = index
    while result < line.count && line[result].isDigit {
            result += 1
        }
    return result - 1
}
...
func findGears(_ matrix: [[Character]]) -> Int {
    var sum = 0
    for i in 0..<matrix.count {
        for j in 0..<matrix[i].count where matrix[i][j].isStar {
            let gearRatio = calculateGearRatio(i: i, j: j, matrix: matrix)
            sum += gearRatio
        }
    }
    return sum
...
func calculateGearRatio(i: Int, j: Int, matrix: [[Character]]) -> Int {
    let adjacentPartNumbers = findAdjacentPartNumbers(i: i, j: j, matrix: matrix)
    return adjacentPartNumbers.count == 2 ? adjacentPartNumbers.reduce(1, *) : 0
}

func findAdjacentPartNumbers(i: Int, j: Int, matrix: [[Character]]) -> [Int] {
    
    return Array(Set<Int>((i - 1...i + 1).flatMap { k in // 3x3 around a star
        (j - 1...j + 1).compactMap { l in
            guard isCorrectIndex(row: k, column: l, matrix: matrix), !(k == i && l == j), matrix[k][l].isDigit else {
                return nil
            }
            return Int(makeNumber(l, matrix, k))
        }
    }))
}
...
func isCorrectIndex(row: Int, column: Int, matrix: [[Character]]) -> Bool {
    return (0..<matrix.count).contains(row) && (0..<matrix[row].count).contains(column)
}

func makeNumber(_ start: Int, _ matrix: [[Character]], _ row: Int) -> String {
    var index = start
    var number = ""
   
    while index - 1 >= 0 && matrix[row][index - 1].isDigit {
        index -= 1
    }
    
    while index < matrix[row].count && matrix[row][index].isDigit {
        number += String(matrix[row][index])
        index += 1
    }
...
extension Character {
    var isStar: Bool { return self == "*" }
    var isDigit: Bool { return self.isNumber }
    var isSymBol: Bool { return self != "." && !self.isNumber }
}
