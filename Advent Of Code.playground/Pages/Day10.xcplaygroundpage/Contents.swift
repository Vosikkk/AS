...
func calculateDistance(_ input: String) -> Int {
    ...
    var queue: [(point: Point, distance: Int)] = [(startPoint, 0)]
    ...
    while !queue.isEmpty {
        ...
        for nbr in getNeighbors(point.x, point.y, lines) {
...
func getNeighbors(_ x: Int, _ y: Int, _ lines: [[Character]]) -> [Point] {
    return getDirectionNeighbors(x, y, lines).map {
        Point(x: x + $0.0, y: y + $0.1)
    }
}


func getDirectionNeighbors(_ x: Int, _ y: Int, _ lines: [[Character]]) -> [(Int, Int)] {
    let directions: [(Int, Int)] = [
        (1, 0), (-1, 0), (0, 1), (0, -1)
    ]
    
    if lines[x][y] == "S" {
        return directions.filter { isValidPoint(x + $0.0, y + $0.1, lines) }
    } else {
        let validDirections: [Character: [(Int, Int)]] = [
            "|": [(1, 0), (-1, 0)],
            "-": [(0, 1), (0, -1)],
            "L": [(-1, 0), (0, 1)],
            "J": [(-1, 0), (0, -1)],
...
func isValidPoint(_ x: Int, _ y: Int, _ lines: [[Character]]) -> Bool {
    return x >= 0 && x < lines.count && y >= 0 && y < lines[0].count
}

func findStartPoint(lines: [[Character]]) -> Point {
